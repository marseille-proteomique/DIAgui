...
get_bestwind <- function(data, n_window = 25, per_frac = FALSE, window_size = NULL){
  ...
  if(is.numeric(window_size)){
    ...
    if(per_frac){
      ...
      for(i in frac){
        ...
        wind <- c()
        for(k in 1:n_window){ # get the windows --> same amount for each
          if(k == 1){
            dep = 1
          }
          else{
      ...
      data$bins <- rep(NA, n)
      for(i in 1:n_window){ # get the windows --> same amount for each
        if(i == 1){
          dep = 1
        }
        else{
